...
class HaravanSnapshotResource(ConfigurableResource):
    """psycopg2 connection to the Postgres holding ``raw_haravan``.

    Reuses the same ``DESTINATION__POSTGRES__CREDENTIALS__*`` env vars as the dlt
    postgres destination, so the snapshot job always reads/writes the same
    database that dlt loads ``raw_haravan.inventory_locations`` into — no
    ...
    def _dsn_kwargs(self) -> dict:
        ...
        if missing:
            raise RuntimeError(
                "HaravanSnapshotResource is missing "
...
__all__ = ["HaravanSnapshotResource"]
